#!/usr/bin/env python3
"""Test with smaller markers for better accuracy."""
import numpy as np
import cv2
...
def test_with_marker_size(ms):
    """Test with specified marker size."""
    print(f"\n{'='*60}")
    print(f"TEST WITH {ms}px MARKERS")
    print("="*60)
    ...
    if valid:
        ...
        print(f"\n  SUMMARY: avg={avg_all:.2f}px, max={max_all:.2f}px")
        return max_all < 5
    return False
...
if __name__ == '__main__':
    print("="*70)
    print("TESTING DIFFERENT MARKER SIZES")
    print("="*70)
    
    for ms in [3, 5, 7, 10]:
        passed = test_with_marker_size(ms)
        if passed:
            print("\n✅ PASSED with < 5px accuracy!")
            break
    else:
        print("\n❌ No marker size achieved < 5px accuracy")
