...
def create_marker_photo(width, height, marker_size=30):
    """Photo with LARGE colored corner markers."""
    photo = np.ones((height, width, 3), dtype=np.uint8) * 200
    
    # Large markers - placed well inside the photo (not at the edge)
    # TL - Red - inset 20px from edges
    inset = 20
    photo[inset:inset+marker_size, inset:inset+marker_size] = [0, 0, 255]
    
    # TR - Green
    photo[inset:inset+marker_size, width-inset-marker_size:width-inset] = [0, 255, 0]
    
    # BR - Blue
    photo[height-inset-marker_size:height-inset, width-inset-marker_size:width-inset] = [255, 0, 0]
    
    # BL - Yellow
    photo[height-inset-marker_size:height-inset, inset:inset+marker_size] = [0, 255, 255]
...
def main():
    ...
    inset = 20  # Where the markers are placed (inset from photo edge)
    
    print("\n" + "="*70)
    print("TESTING WITHOUT EDGE BLUR - Markers INSET from photo edge")
    print("="*70)
    print(f"  Photo: {photo_width}x{photo_height}")
    print(f"  Marker size: 30x30")
    print(f"  Markers inset {inset}px from photo edges")
    print(f"  NO edge blur applied (to isolate polygon calculation error)")
    ...
    for angle in [0, 30, 45, 60, 90, -30]:
        ...
        # NO alpha, NO edge blur - pure test of polygon calculation
        rotated = rotate_photo(photo, angle)
        rgba = cv2.cvtColor(rotated, cv2.COLOR_BGR2BGRA)
        rgba[:, :, 3] = 255
        # DO NOT apply blur_alpha_edges
        ...
        # The polygon corners are at (0,0), (width,0), etc.
        # But the markers are at (inset, inset), etc.
        # So we need to adjust the expected positions
        polygon = get_rotated_polygon_v2(photo_width, photo_height, center_x, center_y, angle)
        
        print(f"\n  {angle:3d}° Rotation:")
        errors = []
        for i in range(4):
            # Expected marker position = polygon corner + inset offset rotated
            expected_marker = polygon[i]
            if i == 0:  # TL
                expected_marker = polygon[i] + np.array([inset, inset])
            elif i == 1:  # TR
                expected_marker = polygon[i] + np.array([-inset, inset])
            elif i == 2:  # BR
                expected_marker = polygon[i] + np.array([-inset, -inset])
            ...
            if i in detected:
                dx = detected[i][0] - expected_marker[0]
                dy = detected[i][1] - expected_marker[1]
                error = np.sqrt(dx**2 + dy**2)
                errors.append(error)
                status = "✅" if error < 5 else "❌"
                print(f"    Corner {i}: calc=({expected_marker[0]:.1f},{expected_marker[1]:.1f}), "
